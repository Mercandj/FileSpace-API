...
class FileController extends \lib\Controller {
	...
	public function get() {
		...
		if($apk_update) {
			...
			if(HTTPRequest::getExist('search'))
				$list_file = $this->getManagerof('File')->getPublic(0, HTTPRequest::getData('search'));
			else
				$list_file = $this->getManagerof('File')->getPublic(0);
		}
		...
		foreach ($list_file as $file) {
			...
			if($file->getDirectory() == 1) {
				$file_array['count'] = $this->getManagerof('File')->getFolderSize($file->getId());
			}
			$result[] = $file_array;
		}
