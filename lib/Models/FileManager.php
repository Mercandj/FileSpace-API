...
class FileManager extends \lib\Manager {
	...
	public function add(File $file) {
		...
		$req = $this->_db->prepare('INSERT INTO file(url,name,size,visibility,date_creation,id_user,type,directory,content) VALUES (:url, :name, :size, :visibility, :date_creation, :id_user, :type, :directory, :content)');
	...
	public function getById($id) {
		$req = $this->_db->prepare('SELECT id,url,name,size,visibility,date_creation,id_user,type,directory,content FROM file WHERE id = :id');
    	$req->bindParam(':id', $id, \PDO::PARAM_INT);
    	$req->execute();
	 ...
	 */
	public function getAll($id_user = 0, $purl="", $psearch = "") {
		$file = [];
		$url = '^'.$purl.'.[^/]*$';
		$search = '%'.$psearch.'%';
		...
		if($id_user == 0) {
			$search = '%'.$search.'%';
			$req = $this->_db->prepare('SELECT id,url,name,size,visibility,date_creation,id_user,type,directory,content,public FROM file WHERE url REGEXP :url AND name LIKE :search ORDER BY date_creation DESC');
			$req->bindParam(':url', $url, \PDO::PARAM_STR);
			...
			$req = $this->_db->prepare('SELECT id,url,name,size,visibility,date_creation,id_user,type,directory,content,public FROM file WHERE url REGEXP :url AND id_user = :id_user AND name LIKE "%'.$search.'%" ORDER BY date_creation DESC');
			$req->bindParam(':url', $url, \PDO::PARAM_STR);
			$req->bindParam(':search', $search, \PDO::PARAM_STR);
			$req->bindParam(':id_user', $id_user, \PDO::PARAM_INT);
			$req->execute();
